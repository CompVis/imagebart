...
from braket.modules.warp.midas import Midas
from braket.modules.transformer.util import Block as TransformerBlock
from braket.modules.transformer.util import TinyTransformer
...
class ParallelAttentionWarper(AbstractWarper):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.pos_block = nn.ModuleList([TinyTransformer(block_size=self.block_size,
                                                        n_layer=2,
                                                        n_head=16,
                                                        n_embd=self.n_embd,
                                                        use_head=False) for _ in range(3)])
        self.emb_block = nn.ModuleList([TinyTransformer(block_size=self.block_size,
                                                        n_layer=2,
                                                        n_head=16,
                                                        n_embd=self.n_embd,
                                                        use_head=False) for _ in range(3)])
    ...
    def _get_embeddings(self, token_embeddings, position_embeddings, warpkwargs):
        ...
        warp_emb = self._to_sequence(warp_emb)
        warp_pos = self._to_sequence(warp_pos)

        cond_emb = self._to_sequence(cond_emb)
        cond_pos = self._to_sequence(cond_pos)

        cond_emb = self.emb_block[0](idx=None, token_embeddings=cond_emb) + cond_emb
        warp_emb = self.emb_block[1](idx=None, token_embeddings=warp_emb) + warp_emb
        new_emb = self.emb_block[2](idx=None, token_embeddings=warp_emb+cond_emb)

        cond_pos = self.pos_block[0](idx=None, token_embeddings=cond_pos) + cond_pos
        warp_pos = self.pos_block[1](idx=None, token_embeddings=warp_pos) + warp_pos
        new_pos = self.pos_block[2](idx=None, token_embeddings=warp_pos+cond_pos)
